...
struct HomeInfoGetService{
    static let shared = HomeInfoGetService()

    func readBestList(completion: @escaping (NetworkResult<Any>) -> (Void)) {
        
        let url = APIConstants.getBookInfoURL
    ...
    private func isValidInfoData(data: Data) -> NetworkResult<Any>{
        ...
        guard let decodeData = try? decoder.decode(BestResponseData.self, from: data)
        else {return .pathErr}
